function [image_coordinates, Disp_map] = run_SparseStereo_one_sided( imgL, imgR, width, height, cam_params, DIFF_THRESHOLD, FEATURE_FACTOR)
max_disparity = 10;
...
%% Settings
feature_window_horizontal = 2; % half window size (one-sided) 2->5
...
image_coordinates = zeros(2000,4);
...
DIFF_L_signed = imsubtract(imgL(:,1:end-2,1),imgL(:,3:end,1));
...
for line = 1+feature_window_vertical:height-feature_window_vertical   
    ...
    for i = 1+feature_window_horizontal+max_disparity: width-feature_window_horizontal-3
...
%             neighbors = [i_first i_first i_first];
            ...
            if (second / first > FEATURE_FACTOR && second > MATCHING_THRESHOLD(i_first) && i_first > 2 ) % check if match peak ratio ( PKR-N ) exceeds minimum threshold
                ...
                disparity = disparity + cam_params(2);
                if DIFF_L_signed(line,i)>0
                    gradient_sign = 1;
                else
                    gradient_sign = -1;
                end
                image_coordinates(disp_cnt,:) = [line ii disparity gradient_sign]; 
                Disp_map(line,ii) = round(disparity*6);
